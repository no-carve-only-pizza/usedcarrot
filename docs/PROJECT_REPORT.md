...
| 악성 상품/사용자 제한 | 관리자가 승인한 신고 기준으로 상품 3회 숨김, 사용자 5회 제한/10회 정지 | `ReportService` |
...
로그인은 Spring Security의 세션 기반 인증을 사용한다. 로그인 실패 메시지는 “이메일 또는 비밀번호가 올바르지 않습니다.”로 일반화했다. 특정 계정을 제3자가 잠그는 공격을 막기 위해 계정 잠금 대신 클라이언트 IP별 60초간 10회 실패 제한을 적용했다.
...
신고 접수만으로는 제재하지 않는다. 관리자가 검토 후 `RESOLVED`로 승인한 신고만 다음 누적 정책에 반영한다.
...
회원가입 시 사용자별 지갑을 0 CC로 생성한다. 무제한 계정 생성에 따른 CC 인플레이션을 막기 위해 자동 지급을 제거하고, 관리자가 사용자 관리 화면에서 필요한 금액만 지급한다. 상품 구매 시 서버에 저장된 상품 가격을 기준으로 구매자 지갑 잔액을 차감하고 판매자 지갑 잔액을 증가시킨다. 거래 완료 후 상품 상태는 `SOLD`로 변경된다.
...
- 로그아웃 시 세션 무효화
- 기본 관리자 계정 제거 및 환경변수 기반 최초 관리자 생성
- IP별 로그인 실패 속도 제한
- 세션 쿠키 `Secure`, `HttpOnly`, `SameSite=Strict` 적용
- CSP 및 프레임 차단 보안 헤더 적용
...
| 공개 기본 관리자 계정 | 소스 내 계정 제거, 환경변수 기반 생성 |
| 다계정 CarrotCoin 무한 생성 | 가입 시 0 CC, 관리자 승인 지급 방식으로 변경 |
| 다계정 허위 신고 자동 제재 | 신고 접수와 제재 분리, 관리자 승인 신고만 누적 |
| 타인 계정 잠금 공격 | 계정 잠금 제거, 클라이언트 IP별 로그인 속도 제한 |
| `X-Forwarded-For` 감사 로그 위조 | 등록한 신뢰 프록시 요청에서만 헤더 사용 |
| 잘못된 HTTP 오류 상태 | 오류 유형별 400/403/404/409 상태 반환 |
...
기본 계정은 생성하지 않는다. 관리자는 환경변수로 최초 생성하고 일반 사용자는 회원가입한다. 운영 환경은 HTTPS와 기본 보안 쿠키 설정을 사용한다.
