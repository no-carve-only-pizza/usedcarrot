...
public class ChatService {
    ...
    public ChatRoom createOrGet(CurrentUser currentUser, Long productId) {
        ...
        Product product = productService.findWithSellerAndImages(productId);
        if (!product.canAcceptMessages()) {
    ...
    @Transactional(readOnly = true)
    public List<Message> messagesAfter(Long roomId, CurrentUser currentUser, Long afterId) {
        room(roomId, currentUser);
        long cursor = afterId == null ? 0L : afterId;
        return messageRepository.findByChatRoomIdAndIdGreaterThanOrderByIdAsc(roomId, cursor);
    }

    @Transactional
    public Message send(Long roomId, CurrentUser currentUser, String content, HttpServletRequest request) {
        ...
        if (!product.canAcceptMessages()) {
            throw new AppException(ErrorCode.INVALID_STATE, "거래가 종료되었거나 삭제된 상품에는 메시지를 보낼 수 없습니다.");
        }
